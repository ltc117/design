...
(function(){
	...
	b.isPointInRect=function(f,g){
		...
		/**
		 * 纠正百度地图获取坐标的偏移量 2017.12.19 lxc
		 */
		var fLat = f.lat + 0.00728;
		var fLng = f.lng - 0.01185;
		
		//console.log("f.lng="+fLng+"   e.lng="+e.lng+"   h.lng="+h.lng);
		//console.log("f.lat="+fLat+"   e.lat="+e.lat+"   h.lat="+h.lat);
		
		return(fLng>=e.lng&&fLng<=h.lng&&fLat>=e.lat&&fLat<=h.lat)
	};
	...
	b.isPointInPolygon=function(o,l){
		...
		var h=t.length;
		var n=true;
		var j=0;var g=2e-10;var s,q;var e=o;s=t[0];
		
		/**
		 * 纠正百度地图获取坐标的偏移量 2017.12.19 lxc
		 * 实际上地图处于不同级别，偏移量有所不同，可根据需要进行偏移量调整。
		 */
		var eLat = e.lat + 0.00728;
		var eLng = e.lng - 0.01185;
		...
		for(var f=1;f<=h;++f){
			...
			q=t[f%h];
			if(eLat<Math.min(s.lat,q.lat)||eLat>Math.max(s.lat,q.lat)){
				s=q;
				continue
			}
			if(eLat>Math.min(s.lat,q.lat)&&eLat<Math.max(s.lat,q.lat)){
				if(eLng<=Math.max(s.lng,q.lng)){
					if(s.lat==q.lat&&eLng>=Math.min(s.lng,q.lng)){
						return n
					}
					if(s.lng==q.lng){
						if(s.lng==eLng){
						...
						var r=(eLat-s.lat)*(q.lng-s.lng)/(q.lat-s.lat)+s.lng;
						if(Math.abs(eLng-r)<g){
							return n
						}
						if(eLng<r){
							++j
				...
				if(eLat==q.lat&&eLng<=q.lng){
					var m=t[(f+1)%h];
					if(eLat>=Math.min(s.lat,m.lat)&&eLat<=Math.max(s.lat,m.lat)){
						++j
					}else{
						j+=2
					}
